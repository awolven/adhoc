		   ...
		   (if component-definition
			 ...
			 (labels ((has-descending? (instance slot-name)
				    (if (member slot-name
						(slot-value
						 (class-of instance)
						 'effective-descending-attributes) :test #'eq)
					(slot-value instance slot-name)
					(if (superior instance)
					    (has-descending? (superior instance) slot-name)
					    (get-toplevel))))
...
(defgeneric adhoc-scene-graph::maybe-refresh-rm-graphics-from-root-in-new-thread (node)
  (:method (node)
    (declare (ignore node))
    (values)))
...
(defmethod update-instance-for-redefined-class :after
  ...
  (labels ((unbind-dependent (object new-slotd old-slotd)
    ...
    (adhoc-scene-graph::maybe-refresh-rm-graphics-from-root-in-new-thread instance)
